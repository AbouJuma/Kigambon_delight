  ...
  <div class="pos_page">
    <div class="container-fluid p-0 app-admin-wrap layout-sidebar-large clearfix" id="pos">
      ...
      <b-row v-if="!isLoading">
            ...
            <div class="main-header">
              <div class="logo">
                <router-link to="/app/dashboard">
                  <img :src="'/images/' + currentUser.logo" alt width="60" height="60" style="text-align: left;">
                </router-link>
              </div>
              ...
              <div class="header-part-right">
                ...
                <!-- Dashboard -->
                <router-link class="i-Back header-icon d-sm-inline-block" to="/app/dashboard" title="Dashboard">
                </router-link>

                <!-- Full screen toggle -->
                <i style="color: #8b5cf6;" title="Full screen"
                  class="i-Full-Screen header-icon d-none d-sm-inline-block" @click="handleFullScreen"></i>

                <!-- Pos Settings -->
                <router-link v-if="currentUserPermissions && currentUserPermissions.includes('pos_settings')"
                  class="i-Data-Settings header-icon d-sm-inline-block" to="/app/settings/pos_settings"
                  title="Pos Settings">
                </router-link>

                <!-- Today's Sales -->
                <i @click="get_today_sales()" style="color: #8b5cf6;" title="Today's Sales"
                  v-if="currentUserPermissions && currentUserPermissions.includes('Sales_view')"
                  class="i-Receipt header-icon d-sm-inline-block"></i>

                <!-- Customer Display Toggle -->
                <i @click="toggleCustomerDisplay()" style="color: #8b5cf6;" 
                  :title="customerDisplay.isEnabled() ? 'Disable Customer Display' : 'Enable Customer Display'"
                  :class="customerDisplay.isEnabled() ? 'i-Monitor header-icon d-sm-inline-block' : 'i-Monitor-Off header-icon d-sm-inline-block'"></i>
                ...
                <div class="dropdown">
                  <b-dropdown id="dropdown" text="Dropdown Button" class="m-md-2" toggle-class="text-decoration-none"
                    ...
                    <template slot="button-content">
                      <i style="color: #8b5cf6!important;" class="i-Globe text-muted header-icon" role="button"
                    ...
                    <b-row>
                      <b-col md="12">
                        <div class="grandtotal">
                          <span>{{ $t("Total_Payable") }} : {{ currentUser.currency }} {{ GrandTotal.toFixed(2)
                            }}</span>
                        </div>
                      </b-col>
        ...
        <b-modal hide-footer size="md" scrollable id="modal_today_sales"
          :title="'Today\'s Sales' + ' ( ' + today_sales.today + ' )'">
          ...
          <b-row class="mb-3">
            ...
            <b-col md="6" class="text-right">
              <h6> {{ currentUser.currency }} {{ formatNumber(today_sales.total_sales_amount, 2) }}</h6>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            ...
            <b-col md="6" class="text-right">
              <h6> {{ currentUser.currency }} {{ formatNumber(today_sales.total_amount_paid, 2) }}</h6>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            ...
            <b-col md="6" class="text-right">
              <h6> {{ currentUser.currency }} {{ formatNumber(today_sales.total_cash, 2) }}</h6>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            ...
            <b-col md="6" class="text-right">
              <h6> {{ currentUser.currency }} {{ formatNumber(today_sales.total_credit_card, 2) }}</h6>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            ...
            <b-col md="6" class="text-right">
              <h6> {{ currentUser.currency }} {{ formatNumber(today_sales.total_cheque, 2) }}</h6>
            </b-col>
          </b-row>
          ...
          <div class="px-3 py-2">
            <b-row>
              <b-col md="12" class="mb-3">
                <b-form-input v-model="search_brand" placeholder="Search Brands" class="w-100"></b-form-input>
              </b-col>
            </b-row>
            ...
            <b-row>
              <div class="col-md-12 d-flex flex-row flex-wrap bd-highlight list-item mt-2">
                ...
                <div class="card o-hidden bd-highlight m-1" v-for="brand in filteredBrands" :key="brand.id"
        ...
        <b-sidebar id="sidebar-category" :title="$t('ListofCategory')" bg-variant="white" right shadow>
          <div class="px-3 py-2">
            <b-row>
              <b-col md="12" class="mb-3">
                <b-form-input v-model="search_category" placeholder="Search Categories" class="w-100"></b-form-input>
              </b-col>
            </b-row>

            <b-row>
              <div class="col-md-12 flex-row flex-wrap bd-highlight list-item mt-2">
                <div @click="getAllCategory()" :class="{ 'brand-Active': category_id == '' }"
                  ...
                  <div class="flex-grow-1 d-bock" style=" cursor: pointer; ">
                    <div class="card-body align-self-center flex-column justify-content-between align-items-lg-center">
                      <div class="item-title">{{ $t('All_Category') }}</div>
                    </div>
                  </div>
                </div>
                <div class="card bd-highlight m-1" v-for="category in filteredCategories" :key="category.id"
        ...
        <b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')">
          <div id="invoice-POS">
            <div style="max-width:400px;margin:0px auto">
              <div class="info">
                <div class="invoice_logo text-center mb-1"
                  style="display: flex; flex-direction: column; align-items: center;">
                  <h6 style="font-size: 10px; margin-bottom: 3px;">***Start of Legal Receipt***</h6>
                  <img :src="'/images/tra3.jfif'" alt="TRA Logo" width="30" height="30">
                </div>

                <div style="font-size: 9px; line-height: 1.2;">
                  <span v-if="comp_details && comp_details.company">
                    <div style="text-align: center; font-weight: bold; margin-bottom: 3px;">
                      {{ comp_details.company.name }}
                    </div>

                    <!-- Compact company details in 2 columns -->
                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                      <span><strong>{{ $t('Adress') }}:</strong> {{ comp_details.company.address }}</span>
                      <span><strong>{{ $t('Mob') }}:</strong> {{ comp_details.company.mobile }}</span>
                    </div>

                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                      <span><strong>{{ $t('TIN') }}:</strong> {{ comp_details.company.TIN }}</span>
                      <span><strong>{{ $t('VRN') }}:</strong> {{ comp_details.company.VRN }}</span>
                    </div>

                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                      <span><strong>{{ $t('Serial Number') }}:</strong> {{ comp_details.company.serial_number }}</span>
                      <span><strong>{{ $t('UIN') }}:</strong> {{ comp_details.company.UIN }}</span>
                    </div>

                    <div style="margin-bottom: 3px;">
                      <strong>{{ $t('Tax Office') }}:</strong> {{ comp_details.company.tax_office }}
                    </div>

                    <div v-show="pos_settings.show_customer" style="margin-bottom: 3px;">
                      <strong>{{ $t('Customer') }}:</strong> {{ invoice_pos.sale.client_name }}
                    </div>
                  </span>

                  <span v-if="receipt_data && receipt_data.receipt">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                      <span><strong>{{ $t('Z No') }}:</strong> {{ receipt_data.receipt.z_no }}</span>
                      <span><strong>{{ $t('Receipt No') }}:</strong> {{ receipt_data.receipt.receipt_no }}</span>
                    </div>
                    <div style="margin-bottom: 3px;">
                      {{ $t('Date') }}: {{ receipt_data.receipt.date }} {{ $t('Time') }}: {{ receipt_data.receipt.time
                      }}
                    </div>
                  </span>
                </div>
              ...
              <!-- Compact items table -->
              <table style="width: 100%; border-collapse: collapse; font-size: 8px; margin: 5px 0;">
                <thead>
                  <tr style="border-top: 1px solid #000; border-bottom: 1px solid #000;">
                    <th style="text-align: left; padding: 2px;">Item</th>
                    <th style="text-align: center; padding: 2px;">Qty</th>
                    <th style="text-align: center; padding: 2px;">Rate</th>
                    <th style="text-align: right; padding: 2px;">Total</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="detail_invoice in invoice_pos.details" style="border-bottom: 1px dotted #ccc;">
                    <td style="padding: 1px 2px; max-width: 120px; word-wrap: break-word;">
                      {{ detail_invoice.name }}
                      ...
                      <span v-show="detail_invoice.is_imei && detail_invoice.imei_number !== null"
                        style="font-size: 7px; color: #666;">
                        IMEI: {{ detail_invoice.imei_number }}
                      </span>
                    </td>
                    <td style="text-align: center; padding: 1px 2px;">
                      {{ formatNumber(detail_invoice.quantity, 1) }}
                    </td>
                    <td style="text-align: center; padding: 1px 2px;">
              ...
              <table style="width: 100%; font-size: 9px; margin-top: 5px;">
                <tbody>
                  <tr v-show="pos_settings.show_discount && invoice_pos.sale.taxe > 0">
                    <td style="text-align: right; padding: 1px 5px 1px 0;">{{ $t('OrderTax') }}:</td>
                    <td style="text-align: right; padding: 1px 0;">
                      {{ invoice_pos.symbol }} {{ formatNumber(invoice_pos.sale.taxe, 2) }}
                      ({{ formatNumber(invoice_pos.sale.tax_rate, 2) }}%)
                    </td>
                  </tr>

                  <tr v-show="pos_settings.show_discount && invoice_pos.sale.discount > 0">
                    <td style="text-align: right; padding: 1px 5px 1px 0;">{{ $t('Discount') }}:</td>
                    <td style="text-align: right; padding: 1px 0;">
                      {{ invoice_pos.symbol }} {{ formatNumber(invoice_pos.sale.discount, 2) }}
                    </td>
                  </tr>

                  <tr v-show="pos_settings.show_discount && invoice_pos.sale.shipping > 0">
                    <td style="text-align: right; padding: 1px 5px 1px 0;">{{ $t('Shipping') }}:</td>
                    <td style="text-align: right; padding: 1px 0;">
                      {{ invoice_pos.symbol }} {{ formatNumber(invoice_pos.sale.shipping, 2) }}
                    </td>
                  </tr>

                  <tr style="border-top: 1px solid #000; font-weight: bold;">
                    <td style="text-align: right; padding: 3px 5px 3px 0; font-size: 11px;">{{ $t('Total') }}:</td>
                    <td style="text-align: right; padding: 3px 0; font-size: 11px;">
                  ...
                  <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal">
                    <td style="text-align: right; padding: 1px 5px 1px 0;">{{ $t('Paid') }}:</td>
                    <td style="text-align: right; padding: 1px 0;">
                      {{ invoice_pos.symbol }} {{ formatNumber(invoice_pos.sale.paid_amount, 2) }}
                    </td>
                  </tr>

                  <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal">
                    <td style="text-align: right; padding: 1px 5px 1px 0;">{{ $t('Due') }}:</td>
                    <td style="text-align: right; padding: 1px 0;">
                      {{ invoice_pos.symbol }} {{ parseFloat(invoice_pos.sale.GrandTotal -
                        invoice_pos.sale.paid_amount).toFixed(2) }}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table v-show="invoice_pos.sale.paid_amount > 0"
                style="width: 100%; font-size: 8px; margin-top: 3px; border-top: 1px dotted #000;">
                <thead>
                  <tr>
                    <th style="text-align: left; padding: 2px;">{{ $t('PayeBy') }}</th>
                    <th style="text-align: center; padding: 2px;">{{ $t('Amount') }}</th>
                    <th style="text-align: right; padding: 2px;">{{ $t('Change') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="payment_pos in payments">
                    <td style="padding: 1px 2px;">{{ payment_pos.Reglement }}</td>
                    <td style="text-align: center; padding: 1px 2px;">{{ formatNumber(payment_pos.montant, 2) }}</td>
                    <td style="text-align: right; padding: 1px 2px;">{{ formatNumber(payment_pos.change, 2) }}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <div style="margin-top: 8px; text-align: center;">
                <p v-show="pos_settings.show_note" style="font-size: 8px; margin: 3px 0;">
                  <strong>{{ pos_settings.note_customer }}</strong>
                </p>

                <div v-show="pos_settings.show_barcode" style="margin: 3px 0;">
                  <barcode class="barcode" :format="barcodeFormat" :value="invoice_pos.sale.Ref" textmargin="0"
                    fontoptions="bold" fontSize="12" height="20" width="1">
                  </barcode>
                </div>

                <div style="margin-top: 5px;">
                  <span v-if="receipt_data && receipt_data.receipt" style="font-size: 9px; font-weight: bold;">
                    {{ receipt_data.receipt.fiscal_code }}
                  </span>

                  <img v-if="receipt_data && receipt_data.counter && receipt_data.counter.qr_code_image"
                    :src="'data:image/svg+xml;base64,' + receipt_data.counter.qr_code_image" alt="QR Code"
                    style="margin: 3px 0; max-width: 80px; display: block; margin: 3px auto;">

                  <span style="font-size: 9px; font-weight: bold;">
                    {{ $t('***End Of Legal Receipt***') }}
                  </span>
                </div>
              </div>
            </div>
        ...
        <b-modal hide-footer size="lg" id="show_draft_sales" title="Draft Sales">
  ...
  methods: {
    ...
    add_product(code) {
      ...
      if (this.details.some(detail => detail.code === code)) {
        ...
        // Show item on customer display briefly
        if (this.product.name && this.product.Total_price) {
          this.showItemOnDisplay(this.product.name, parseFloat(this.product.Total_price));
        }
    ...
    Invoice_POS(id) {
      ...
      axios
        ...
        .then(response => {
          ...
          this.comp_details = response.data.comp_details || {};
    ...
    CaclulTotal() {
      ...
      // Update customer display with new total
      this.updateCustomerDisplay(this.GrandTotal);
    },
    //-------Verified QTY
    ...
    Reset_Pos() {
      ...
      // Clear customer display when cart is reset
